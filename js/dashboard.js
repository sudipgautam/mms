//
//    Main script of dashboard v1.0 Bootstrap Theme
//
"use strict";
/*-------------------------------------------
...
function OpenModalBox(header, inner, bottom){
	...
	modalbox.find('.dashboard-modal-inner').html(inner);
	modalbox.find('.dashboard-modal-bottom').html(bottom);
...
function CloseModalBox(){
	...
	modalbox.fadeOut('fast', function(){
		...
		modalbox.find('.dashboard-modal-inner').children().remove();
		modalbox.find('.dashboard-modal-bottom').children().remove();
...
function DrawCalendar(){
	...
	var calendar = $('#calendar').fullCalendar({
		...
		select: function(start, end, allDay) {
			var form = $('<form id="bill_form">'+
				'<div class="form-group has-success has-feedback">'+
				'<label">Add Bills</label>'+
				'<div>'+
				'<input type="text" id="bill_title" class="form-control" required placeholder="Bill Title">'+
				'</div>'+
				'<label>Description</label>'+
				'<div>'+
				'<textarea rows="3" id="bill_desc" class="form-control" placeholder="Bill Description"></textarea>'+
				'</div>'+
				'</div>'+
				'</form>');
			var buttons = $('<button id="bill_cancel" type="cancel" class="btn btn-default btn-label-left">'+
							'<span><i class="fa fa-clock-o txt-danger"></i></span>'+
							'Cancel'+
							'</button>'+
							'<button type="submit" id="bill_submit" class="btn btn-primary btn-label-left pull-right">'+
							'<span><i class="fa fa-clock-o"></i></span>'+
							'Add'+
							'</button>');
			OpenModalBox('Add Bills', form, buttons);
			$('#bill_cancel').on('click', function(){
				CloseModalBox();
			});
			$('#bill_submit').on('click', function(){
				var new_event_name = $('#bill_title').val();
				if (new_event_name != ''){
					calendar.fullCalendar('renderEvent',
						{
							title: new_event_name,
							description: $('#bill_desc').val(),
							start: start,
							end: end,
							allDay: allDay
						},
...
//////////////////////////////////////////////////////
//
//      MAIN DOCUMENT READY SCRIPT OF dashboard THEME
//
...
$(document).ready(function () {
	...
	if (ajax_url.length < 1) {
		ajax_url = 'ajax/get_overview.php';
	}
